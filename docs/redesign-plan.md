...
- [x] Боковая колонка быстрого выбора: список / витрина / категории / клавиатура
      ...
      - Витрина — это «все товары», а не набор: как у десктопа, карточки
        берутся из остатков точки. Локальной базы у мобилки нет, поэтому и
        весь список, и поиск по нему идут на сервер тем же запросом, что и
        каталог. Партии схлопываем по штрих-коду — плитка на товар одна
      - Тот же запрос даёт цены позициям набора: сам набор цен не отдаёт
      ...
      - Тесты: `test/quick_rail_test.dart` (18), всего 460 зелёных

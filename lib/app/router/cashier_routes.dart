...
List<RouteBase> cashiers = [
  GoRoute(
    ...
    pageBuilder: (context, state) {
      ...
      return nativePageBuilder(context, state, ChequeDetail(id: id));
    },
  ),
  GoRoute(
    path: '/payment',
    pageBuilder: (context, state) => nativePageBuilder(context, state, PaymentSample()),
  ),
  GoRoute(
    path: '/profile/x-report',
    pageBuilder: (context, state) => nativePageBuilder(context, state, XReport()),
  ),
  GoRoute(
  ...
  GoRoute(
    ...
    pageBuilder: (context, state) => nativePageBuilder(context, state, QuickSelection()),
  ),
  GoRoute(
    path: '/profile/quick-selection/picker',
    pageBuilder: (context, state) {
      final added = state.extra is Set<String> ? state.extra as Set<String> : <String>{};
      return nativePageBuilder(context, state, QuickSelectionPicker(addedBarcodes: added));
    },
  ),
  GoRoute(
    path: '/profile/settings',
    pageBuilder: (context, state) => nativePageBuilder(context, state, Settings()),
  ),
  GoRoute(
    path: '/profile/whats-new',
    pageBuilder: (context, state) => nativePageBuilder(context, state, WhatsNew()),
  ),
  GoRoute(
    path: '/profile/logs',
    pageBuilder: (context, state) => nativePageBuilder(context, state, Logs()),
  ),
  GoRoute(
    path: '/return',
    pageBuilder: (context, state) => nativePageBuilder(context, state, Return()),
  ),
];
